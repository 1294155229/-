<template>
	<view class="">
		<u-navbar title="忘记密码" :borderBottom="false" immersive isFixed :background="{ background: 'rgba(0, 0, 0, 0)' }"></u-navbar>
		...
		<view class="container">
			...
			<view class="btn" @tap="userBindMobile">提交</view>
...
export default {
	...
	methods: {
		...
		userBindMobile() {
			...
			this.$http.userForgetPwd(data).then(res => {
				if (res.code == 1) {
		...
		getCode() {
			...
			this.$http.sendSms(data).then(res => {
				if (res.code == 1) {
					this.$helper.showS('发送成功: ' + res.data);
				}
			});
		},
