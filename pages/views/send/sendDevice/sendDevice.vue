	...
	<view>
		<view class="form u-border-top">
			<view class="form-row u-border-bottom" v-if="list">
				<view class="label-box"><view class="label">设备分类</view></view>
		...
		<view class="form">
			...
			<textarea class="textarea u-border" value="" placeholder="多行输入" placeholder-style="color:#BFBFBF" v-model="form.intro" />
		</view>
		<view class="form">
...
export default {
			...
			if (res.code == 1) {
				...
				if (that.bjstate) {
					res.data.map((item, index) => {
						// console.log(item.id,lassifyIds)
	...
	methods: {
		...
		adddata() {
			...
			data.contactWay.length == 11 ? '' : (name = name + '联系方式/');
