<template>
	<view>
		<view class="form u-border-top">
			<view class="form-col u-border-bottom">
				<view class="form-col-title">标题</view>
				<input class="form-col-input" type="text" placeholder="请输入" placeholder-style="color:#BFBFBF" v-model="form.title" />
			</view>

			<view class="form-row u-border-bottom">
				<view class="label-box"><view class="label">联系人</view></view>
				<view class="right-box"><input class="input" type="text" placeholder="请输入" placeholder-style="color:#BFBFBF" v-model="form.contactName" /></view>
			</view>
			...
			<view class="form-row u-border-bottom" v-if="list">
				<view class="label-box"><view class="label">分类</view></view>
		...
		<view class="form">
			<view class="form-row u-border-bottom"><view class="title">求购内容</view></view>
			<textarea class="textarea u-border" value="" placeholder="多行输入" placeholder-style="color:#BFBFBF" v-model="form.remark" />
		</view>
		<view class="uplodImg">
			<view class="imgs" v-for="(it, i) in weighingImg" :key="i" @click="aoutaddimg(i)">
...
export default {
	...
	methods: {
		...
		adddata() {
			this.$u.debounce(() => {
			let name = '';
			this.form.title ? '' : (name = name + '标题/');
			this.form.contactName ? '' : (name = name + '姓名/');
			this.form.contactWay.length == 11 ? '' : (name = name + '联系方式/');
			this.form.remark ? '' : (name = name + '求购内容/');
					...
					uni.uploadFile({
						...
						success: resp => {
							...
							if (index == 1) {
								...
								// this.addImgLists.push(JSON.parse(resp.data).data);
								// this.carImg = this.addImgLists.toString();
								that.weighingImg.push(that.$http.IMG_BASE_URL + datas.data);
							}
						},
						fail: err => {
							fail++;
